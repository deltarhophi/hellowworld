...
def get_connection():
    ...
    conn = psycopg2.connect(
        host=os.getenv("NF_HELLO_POSTGRESQL_HOST") or os.getenv("DB_HOST"),
        port=os.getenv("NF_HELLO_POSTGRESQL_PORT") or os.getenv("DB_PORT", "5432"),
        dbname=os.getenv("NF_HELLO_POSTGRESQL_DATABASE") or os.getenv("DB_NAME"),
        user=os.getenv("NF_HELLO_POSTGRESQL_USERNAME") or os.getenv("DB_USER"),
        password=os.getenv("NF_HELLO_POSTGRESQL_PASSWORD") or os.getenv("DB_PASSWORD"),
        # Si TLS est activé, on demande à psycopg2 d’utiliser SSL.
        sslmode="require" if os.getenv("NF_HELLO_POSTGRESQL_TLS_ENABLED", "false").lower() == "true"
        else "disable",
    )
    return conn
